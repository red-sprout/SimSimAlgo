...
export async function notifyDiscordAuthFailure(filePath: string, webhookUrl: string | undefined): Promise<void> {
  ...
  const response = await fetch(webhookUrl, { method: "POST", headers: { "content-type": "application/json" }, body: JSON.stringify({ content: `@everyone ⚠️ SimSimAlgo AtCoder 세션 인증 실패\n${new Date(now).toISOString()}\nREVEL_SESSION을 갱신한 뒤 n8n workflow를 다시 실행해 주세요.`, allowed_mentions: { parse: ["everyone"] } }), signal: AbortSignal.timeout(10_000) });
